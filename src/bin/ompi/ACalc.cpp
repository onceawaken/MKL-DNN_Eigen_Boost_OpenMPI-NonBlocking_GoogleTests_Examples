...
void ACalc_t::compute(
	...
	SDPPR3(myrank, "CPU run kernels (elemId: %d)\n", itemIdem->id);
...
ACalc_t::ACalc_t(int myrank, size_t nConst_, size_t nSteps_, size_t nIdem_, double t0, double t1) :
		...
		vecAns(nConst_*nSteps_, dummyAns) {
	...
	if (nIdem < 1) {
		PPR( "ACalc_t::ACalc_t ERROR: nIdem < 1\n");
		exit(-1);
	}
	if (nSteps < 1) {
		PPR( "ACalc_t::ACalc_t ERROR: nSteps < 1\n");
		exit(-1);
	}
	if (nConst < 1) {
		PPR( "ACalc_t::ACalc_t ERROR: nConst < 1\n");
		exit(-1);
	}

	std::cout << "sizeof(vecAns) : " << sizeof(vecAns) << std::endl;
	std::cout << "sizeof(vecSum) : " << sizeof(vecSum) << std::endl;
	std::cout << "sizeof(vecAnsInit) : " << sizeof(vecAnsInit) << std::endl;


	if (vecAns.size() != nConst * nSteps) {
		PPR("ACalc_t::ACalc_t WARNING: CURR SIZE (vecAns.GRID_SIZE():%zu) DIFFER (nConst*nSteps:%zu)\n",
		        vecAns.size(), nConst * nSteps);

		PPR( "ACalc_t::ACalc_t WARNING: NEW SIZE  (vecAns.GRID_SIZE():%zu) == (nConst*nSteps:%zu)\n",
		        vecAns.size(), nConst * nSteps);
	}
