...
MpiMaster::MpiMaster(int myrank, int proccount,  AData_t *oDataGen, ACalc_t *oResults) :
		myrank(myrank), proccnt(proccount), oDataGen(oDataGen), oResults(oResults)
		{
...
int MpiMaster::send_const() {
	...
	for (int i = 1; i < proccnt; i++) {
		...
		MDPPR1("(%d) to SLAVE (%d) send_const 2 : vec const GRID_SIZE(items: %d)\n", ++cnt, i, oDataGen->size_itemsC());
